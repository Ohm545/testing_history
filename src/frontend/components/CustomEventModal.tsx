...
export const CustomEventModal: React.FC<CustomEventModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-2xl bg-[#0F172A] border border-gray-800 rounded-2xl shadow-2xl overflow-hidden my-8">
        {/* Header */}
        <div className="p-5 border-b border-gray-800 flex items-center justify-between bg-gray-950/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Manual Custom Event Generator</h3>
              <p className="text-xs text-gray-400">Directly dispatch a customized synthetic event to POST /events</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-lg bg-gray-950 border border-gray-800 text-xs text-white focus:outline-none focus:border-indigo-500"
              >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 rounded-lg bg-gray-950 border border-gray-800 font-mono text-xs text-cyan-300 focus:outline-none focus:border-indigo-500"
            />
          ...
          {apiResponse && (
            <div className="p-3 rounded-xl bg-gray-950 border border-gray-800 font-mono text-xs">
              <div className="flex items-center gap-2 mb-1">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />
                <span className="text-emerald-400 font-bold">API Ingestion Response (HTTP 201)</span>
              </div>
              <pre className="text-gray-300 overflow-x-auto text-[11px]">
          ...
          <div className="pt-3 border-t border-gray-800 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-xs font-bold text-white shadow-lg shadow-indigo-600/30 transition-all disabled:opacity-50"
            >
              {loading ? (
                <>
